import {
  ...
  import {
    PaymentMethodEntity,
  } from "../orm_models/paymentMethod.entity";
  ...
  export const paymentMethodRepoBase = (dbConnection: DataSource | QueryRunner) => ({
    manager: dbConnection.manager,
  
    async findOne(findData: FindOneOptions<PaymentMethodEntity>): Promise<IPaymentMethod> {
      const paymentMethod = await this.manager.findOne(PaymentMethodEntity, findData);
    ...
    async findAll(
      ...
      const paymentMethods = await this.manager.find(PaymentMethodEntity, findData);
      return this.toDomainPaymentMethods(paymentMethods);
    },
  
    async createPaymentMethod(payload: ICreatePaymentMethodInput): Promise<IPaymentMethod> {
    ...
    async deletePaymentMethod(paymentMethod: IPaymentMethod): Promise<number> {
      const result = await this.manager.softDelete(PaymentMethodEntity, paymentMethod.id);
    ...
    async updatePaymentMethod(
      ...
      const updatedPaymentMethod = await this.manager.findOne(PaymentMethodEntity, {
        where: {
