...
class ApiFeatures {
    ...
    static generateJsonObject(tables, joinInfo) {
        ...
        const objectProperties = tables.map((table, index) => {
            ...
            ((_e = joinInfo === null || joinInfo === void 0 ? void 0 : joinInfo[table]) === null || _e === void 0 ? void 0 : _e.foreignConditionField)
                ? (isJsonArray = true)
                : isJsonArray;
            let tableFields;
        ...
        return isJsonArray
            ...
            : `JSON_OBJECT(${objectProperties.join()}${(0, exports.parenthesesGenerator)(tables.length)} AS ${((_a = joinInfo === null || joinInfo === void 0 ? void 0 : joinInfo[tables[0]]) === null || _a === void 0 ? void 0 : _a.asTableAlias)
                ? (_b = joinInfo === null || joinInfo === void 0 ? void 0 : joinInfo[tables[0]]) === null || _b === void 0 ? void 0 : _b.tableAlias
                : tables[0]}`;
    }
    ...
    static generateSqlStatement(tableName, joinClause, whereSql, filterWhereSql, jsonObject, queryParams, pagination, apiOptions) {
        ...
        return `SELECT ${toSelectFields}${apiOptions.isPaging ? `, COUNT(1) OVER() as count` : ""}${jsonObj.join() ? `, ${jsonObj.join()}` : ""} FROM ${tableName} ${joins.join(" ")}
         WHERE ${deleted} ${filterWhereSql ? `AND (${filterWhereSql})` : ""} ${whereSql ? `AND (${whereSql})` : ""} ${apiOptions.isGroupedById ? `GROUP BY ${tableName}.id` : ""} ${orderBy ? `ORDER BY ${orderBy} ${sortedBy}` : ""} ${apiOptions.isPaging
