...
export type MailUserUseCaseType<T> = (
  user: IUser,
  context: T
) => Promise<boolean>;
...
export const mailUserUseCaseBase =
  () =>
  (payload: { subject: string; template: string }) =>
  async (user: IUser, context: any) => {
    try {
      // const email =
...
export const mailUserUseCaseBuilder: MailUserUseCaseBuilderType =
  mailUserUseCaseBase();
